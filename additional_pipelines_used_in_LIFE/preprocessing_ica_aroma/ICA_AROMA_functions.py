...
#FB: modified return values of functions and numbering of denoising types 
...
def runICA(fslDir, inFile, outDir, melDirIn, mask, dim, TR):
	...
	returns filename of melodic_IC_thr.nii.gz"""
	...
	return melICthr,melICmix,melodic_FTmix
...
def feature_spatial(fslDir, tempDir, aromaDir, melIC):
	...
	for i in range(0,numICs):
		...
		mask_csf_loc='-k '+aromaDir+"mask_csf.nii.gz"
		...
		mask_edge_loc='-k '+aromaDir+"mask_edge.nii.gz"
		...
		mask_out_loc='-k '+aromaDir+"mask_out.nii.gz"
...
def denoising(fslDir, inFile, outDir, melmix, denType, denIdx):
	...
	outDir:		Full path of the output directory
	melmix:		Full path of the melodic_mix text file
	denType:	Type of requested denoising ('1': aggressive, '2': non-aggressive, '3': both aggressive and non-aggressive 
	...
	return os.path.join(outDir,'denoised_func_data_nonaggr.nii.gz')